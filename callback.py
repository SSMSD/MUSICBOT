...
from SURAJM import YouTube, app
from SURAJM.core.call import SURAJ
from SURAJM.misc import SUDOERS, db
from SURAJM.utils import bot_sys_stats
from SURAJM.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from SURAJM.utils.formatters import seconds_to_min
from SURAJM.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from SURAJM.utils.stream.autoclear import auto_clean
from SURAJM.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await SURAJ.pause_stream(chat_id)
        ...
        await SURAJ.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            try:
                ...
                return await SURAJ.stop_stream(chat_id)
            except:
                return
        await CallbackQuery.answer()
        queued = check[0]["file"]
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await SURAJ.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
